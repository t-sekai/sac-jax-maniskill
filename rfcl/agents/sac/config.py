...
class SACConfig:
    ...
    share_ve: Optional[bool] = True
    """
    Whether to share the visual encoder between the actor and the critics. 
    If False, then the actor and critics will be using separate visual encoder.
    If True, then the actor and critics will be sharing the same visual encoder.
    """
    actor_update_shared_ve: Optional[bool] = False
    """
    Whether to update the visual encoder with actor updates when using a shared visual encoder between the actor and critics.
    """
    save_ckpt: Optional[bool] = True
    """
    Whether to save checkpoint.
    """
    load_ckpt: Optional[bool] = False
    """
    Whether to load a saved checkpoint.
    """
    load_params_only: Optional[bool] = True
    """
    Whether we load only the params and not continue training with checkpoint logger and other info. If load_params=False, everything from the checkpoint will be loaded.
    """
    load_ve_only: Optional[bool] = False
    """
    If true and load_params=True, load only the visual encoder.
    """
    ckpt_path: Optional[str] = None
    """
    Specify the checkpoint path if loading checkpoint model.
    """
@struct.dataclass
class TimeStep:
